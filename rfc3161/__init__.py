...
def get_hash_oid(hashname):
    return constants.__dict__['id_' + hashname]


def get_hash_from_oid(oid):
    h = constants.oid_to_hash.get(oid)
    if h is None:
...
def get_timestamp(tst):
    try:
        if not isinstance(tst, types.TimeStampToken):
            tst, substrate = decoder.decode(tst, asn1Spec=types.TimeStampToken())
            if substrate:
        ...
        if substrate:
            raise ValueError("extra data after tst")
        tstinfo, substrate = decoder.decode(tstinfo, asn1Spec=types.TSTInfo())
        if substrate:
            raise ValueError("extra data after tst")
...
def check_timestamp(tst, certificate, data=None, digest=None, hashname=None, nonce=None):
    ...
    # check content type
    if tst.content['contentInfo']['contentType'] != constants.id_ct_TSTInfo:
        return False, "Signed content type is wrong: %s != %s" % (
            tst.content['contentInfo']['contentType'], constants.id_ct_TSTInfo)
    ...
    # print "contentInfo\t:"
    # print "\tsigner_hash_name:", signer_hash_name
    # print "\tcontent_digest\t:", content_digest.encode('hex')
    # print "\thashed_message\t:", str(message_imprint.hashed_message).encode('hex')
    # check signature
    ...
    if m2crypto_available :
        ...
        return True, 'Signature not checked: M2Crypto library not installed'
...
class RemoteTimestamper(object):
    def __init__(self, url, certificate=None, capath=None, cafile=None, username=None, password=None, hashname=None,
                 include_tsa_certificate=False, keystore=None, truststore=None):
        self.url = url
        ...
        if keystore is not None:
            self.transport = httpslib.HTTPSClientCertTransport(keystore, keystore, truststore)
        else:
            self.transport = None
    ...
    def getpeercert(self):
        return self.transport.getpeercert() if self.transport is not None else None
    ...
    def __call__(self, data=None, digest=None, include_tsa_certificate=None, nonce=None):
        ...
        message_imprint = types.MessageImprint()
        ...
        request.setComponentByPosition(4,
                                       include_tsa_certificate if include_tsa_certificate is not None else self.include_tsa_certificate)
        ...
        try:
            if(self.transport is not None):
                response = self.transport.u2open(http_request).read()
            else:
        ...
        try:
            tst_response, substrate = decoder.decode(response, asn1Spec=types.TimeStampResp())
        except:
        ...
        if result:
            return encoder.encode(tst_response.time_stamp_token), message
        else:
